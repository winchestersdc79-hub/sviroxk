...
export function TaskCard({ task, quadrantColor, onComplete, onPress, onPin }: TaskCardProps) {
  ...
  ].filter(q => q !== task.quadrant) as TaskQuadrant[];
  ...
  return (
    <>
      <Pressable
        ...
        style={({ pressed }) => [styles.card, { backgroundColor: colors.card, borderLeftColor: quadrantColor, opacity: task.isCompleted ? 0.55 : pressed ? 0.85 : 1 }]}
      >
        <View style={styles.row}>
          ...
          <View style={styles.content}>
            ...
            {task.description ? <Text style={[styles.description, { color: colors.mutedForeground }]} numberOfLines={1}>{task.description}</Text> : null}
      ...
      <Modal visible={menuVisible} transparent animationType="fade" onRequestClose={() => setMenuVisible(false)}>
        <Pressable style={styles.overlay} onPress={() => setMenuVisible(false)}>
          <View style={[styles.menu, { backgroundColor: colors.card, shadowColor: "#000" }]}>
            ...
            <Pressable style={styles.menuItem} onPress={() => { setMenuVisible(false); setTimeout(() => setEditVisible(true), 300); }}>
            ...
            <Pressable style={styles.menuItem} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); onPin?.(); setMenuVisible(false); }}>
              ...
              <Text style={[styles.menuItemText, { color: task.isPinned ? colors.primary : colors.foreground }]}>{task.isPinned ? "Открепить" : "Закрепить"}</Text>
            </Pressable>
